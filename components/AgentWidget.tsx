...
const NetworkWarning = () => (
  <div className="absolute inset-0 z-[100] flex items-center justify-center pointer-events-none p-4">
    <div className="bg-amber-600 text-white text-[12px] font-black uppercase tracking-tight px-4 py-3 rounded-2xl shadow-[0_15px_40px_rgba(0,0,0,0.6)] flex flex-col items-center gap-2 border-2 border-amber-400 backdrop-blur-xl animate-pulse text-center leading-none ring-4 ring-amber-600/20">
      ...
      <span className="whitespace-nowrap text-[13px] tracking-widest">Network Not Stable</span>
    </div>
  </div>
);
...
export const AgentWidget: React.FC<AgentWidgetProps> = ({ agentProfile, apiKey, isWidgetMode, onSessionEnd }) => {
  ...
  useEffect(() => {
    ...
    const checkNetwork = () => {
      ...
      const isSlow = conn.effectiveType === '2g' || 
                     ...
                     (conn.rtt && conn.rtt > 550) || 
                     (conn.downlink && conn.downlink < 0.8);
  ...
  const analyzeAndSendReport = useCallback(async (recording: Omit<Recording, 'id' | 'url'>) => {
    ...
    try {
        ...
        if (recording.transcript) {
            contents = { parts: [
                { text: `Analyze this session transcript. Provide summary, sentiment, and action items as JSON.` },
                { text: `TRANSCRIPT:\n${recording.transcript}` }
            ] };
        } else if (recording.blob) {
            const audioBase64 = await blobToBase64(recording.blob);
            contents = { parts: [
                { text: `Analyze this audio. Provide summary, sentiment, and action items as JSON.` },
                { inlineData: { mimeType: recording.mimeType, data: audioBase64 } },
            ] };
        } else {
             contents = { parts: [{ text: "Empty session." }] };
        }
        ...
        try {
            ...
            if (response.text) {
             ...
             console.error("Analysis failed:", geminiError);
        }
  ...
  const handleChatMessage = async (text: string) => {
    ...
    try {
        ...
        setMessages(prev => [...prev, { role: 'model', text: "Connection issues. Please try again.", timestamp: new Date() }]);
    } finally {
  ...
  const endChatSession = useCallback(() => {
    ...
    if (isWidgetMode && onSessionEnd) {
       onSessionEnd({ ...newRecording, id: `chat-${now.getTime()}`, url: '' });
    }
  ...
  const startVoiceSession = useCallback(async () => {
    ...
    try {
      ...
      setErrorMessage("Microphone Denied. Please check settings.");
      cleanupServices();
      return;
    } finally {
        ...
        try {
            ...
            const response = await ai.models.generateContent({
                ...
                config: {
                    ...
                    speechConfig: {
                        voiceConfig: { prebuiltVoiceConfig: { voiceName: (agentProfile as AgentConfig).voice } },
                    },
                },
            });
    ...
    geminiServiceRef.current = new GeminiLiveService(apiKey, agentProfile as AgentConfig, {
      onStateChange: (state) => {
        ...
        if (state === 'ended') {
            if (widgetStateRef.current === WidgetState.Connecting) {
                setWidgetState(WidgetState.Error);
                setErrorMessage("NETWORK ERROR: FAILED TO CONNECT.");
            } else {
      ...
      onTranscriptUpdate: (isFinal, text, type) => {
        ...
        if (isFinal && type === 'output') {
          ...
          const endKeywords = ['goodbye', 'farewell', 'take care', 'talk to you later', 'bye bye', 'bye', 'ese pupo'];
  ...
  const handleBack = () => {
      if (view === 'chat') endChatSession();
          ...
          if (widgetState !== WidgetState.Idle && widgetState !== WidgetState.Ended) endVoiceSession();
          setView('home');
      }
  };
  ...
  useEffect(() => {
    ...
    if (isOpen) { width = 400; height = 600; }
    else if (showCallout) { width = 250; height = 220; }
  ...
  useEffect(() => {
    ...
    if (!isOpen && !calloutDismissed && agentProfile.calloutMessage) setShowCallout(true);
        ...
        {!isOpen ? (
            <div className="relative group flex items-end justify-end h-full p-4">
                {showCallout && agentProfile.calloutMessage && (
                  <div className="absolute bottom-[calc(100%+8px)] right-4 w-[220px] px-5 py-3 bg-white dark:bg-gray-800 rounded-2xl shadow-xl animate-fade-in-up border border-gray-100 dark:border-gray-700 z-[10000]">
                    <button onClick={handleDismissCallout} className="absolute top-2 right-2 text-gray-400 p-1"><svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
                ...
                <button onClick={toggleWidget} className={`w-16 h-16 rounded-full bg-accent-${accentColorClass} shadow-2xl flex items-center justify-center text-white transform hover:scale-110 transition-all animate-pulse active:scale-95`}><FabIcon /></button>
            </div>
        ) : (
            <div className={`flex flex-col w-full h-full bg-white dark:bg-gray-900 md:rounded-[2rem] overflow-hidden border-0 shadow-2xl relative`}>
                {view === 'home' && (
                    <div className="flex flex-col h-full w-full animate-fade-in-up">
                        <button onClick={toggleWidget} className="absolute top-5 right-5 z-50 p-2.5 rounded-full bg-white/10 hover:bg-white/20 text-white transition-all backdrop-blur-md active:scale-90"><svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3"><path d="M6 18L18 6M6 6l12 12" /></svg></button>
                        ...
                        <div className="flex-1 bg-gray-50 dark:bg-gray-900 relative -mt-6 rounded-t-[2rem] px-6 pt-8 flex flex-col gap-4 shadow-2xl">
                            ...
                            <form onSubmit={(e) => { e.preventDefault(); if(chatInput.trim()) initChat(chatInput); }} className="relative">
                                <input type="text" placeholder="Ask a question..." value={chatInput} disabled={!isOnline} onChange={(e) => setChatInput(e.target.value)} className={`w-full pl-6 pr-14 py-4 rounded-2xl bg-white dark:bg-gray-800 shadow-sm border-2 border-transparent focus:border-accent-${accentColorClass} font-semibold`} />
                                <button type="submit" disabled={!isOnline} className="absolute right-3 top-1/2 -translate-y-1/2 p-3 text-gray-500"><SendIcon /></button>
                            </form>
                            <button onClick={startVoiceSession} disabled={!isOnline} className={`w-full bg-gradient-to-r from-accent-${accentColorClass} to-gray-800 rounded-2xl p-1 shadow-xl hover:scale-[1.03] transition-transform`}>
                                <div className="bg-white/10 backdrop-blur-md rounded-xl p-5 flex items-center gap-5"><div className="p-4 bg-white/20 rounded-2xl animate-pulse"><MicrophoneIcon state={WidgetState.Idle} /></div><div className="text-left text-white"><h3 className="font-black text-xl uppercase leading-none">Talk to AI</h3><p className="text-[10px] font-bold opacity-80 mt-1 uppercase tracking-widest">Voice Assistant Ready</p></div></div>
                            </button>
                        </div>
                    </div>
                )}
                    ...
                    <div className="flex flex-col h-full w-full animate-fade-in-up">
                        ...
                        <div className="flex-grow overflow-y-auto p-4 space-y-4 bg-gray-50 dark:bg-gray-900">
                            ...
                            {messages.map((msg, idx) => (
                                <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[85%] rounded-2xl p-4 text-[15px] shadow-sm ${msg.role === 'user' ? `bg-accent-${accentColorClass} text-white rounded-br-none` : 'bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 rounded-bl-none font-semibold'}`}>{msg.text}</div></div>
                            ))}
                            {isChatTyping && <div className="flex justify-start"><div className="bg-white dark:bg-gray-800 rounded-2xl p-4 flex gap-1"><div className="w-1.5 h-1.5 bg-gray-400 rounded-full animate-bounce"></div><div className="w-1.5 h-1.5 bg-gray-400 rounded-full animate-bounce [animation-delay:0.2s]"></div><div className="w-1.5 h-1.5 bg-gray-400 rounded-full animate-bounce [animation-delay:0.4s]"></div></div></div>}
                            <div ref={messagesEndRef} />
                        </div>
                        <form onSubmit={(e) => { e.preventDefault(); if(chatInput.trim()) handleChatMessage(chatInput); }} className="p-4 border-t border-gray-100 dark:border-gray-700"><div className="relative"><input type="text" value={chatInput} disabled={!isOnline} onChange={(e) => setChatInput(e.target.value)} placeholder="Type here..." className="w-full pl-5 pr-14 py-4 rounded-2xl bg-gray-100 dark:bg-gray-800 focus:outline-none focus:border-accent-${accentColorClass} font-semibold" /><button type="submit" disabled={!chatInput.trim() || !isOnline} className={`absolute right-2 top-1/2 -translate-y-1/2 p-3 text-white bg-accent-${accentColorClass} rounded-xl shadow-md`}><SendIcon /></button></div></form>
                    </div>
                )}
                {view === 'voice' && (
                    <div className="flex flex-col h-full w-full animate-fade-in-up">
                        ...
                        <div className="flex-grow flex flex-col items-center justify-center relative bg-white dark:bg-gray-900">
                            ...
                            <div className="relative w-full flex items-center justify-center mb-10 min-h-[220px]">
                                ...
                                <div className={`relative w-56 h-56 rounded-full bg-gradient-to-br from-accent-${accentColorClass} to-gray-400 dark:to-gray-800 shadow-2xl flex items-center justify-center transition-transform duration-700 ${widgetState === WidgetState.Speaking ? 'scale-110' : 'scale-100'}`}>
                                    <div className={`relative w-52 h-52 rounded-full flex items-center justify-center shadow-inner z-10 overflow-hidden bg-white dark:bg-gray-900 ${widgetState === WidgetState.Ended ? `!bg-accent-${accentColorClass}` : ''}`}>
                                        {widgetState === WidgetState.Connecting && <Spinner className={`w-24 h-24 text-accent-${accentColorClass}`} />}
                                        {(widgetState === WidgetState.Listening || widgetState === WidgetState.Speaking || widgetState === WidgetState.Idle) && <div className={`transition-transform duration-500 ${widgetState === WidgetState.Speaking ? 'scale-125' : 'scale-100'}`}><WaveformIcon className={`h-28 w-28 ${widgetState === WidgetState.Idle ? 'text-gray-200' : `text-accent-${accentColorClass}`}`} /></div>}
                                        {widgetState === WidgetState.Error && <div className="text-red-500 animate-pulse"><svg className="h-24 w-24" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></div>}
                                        {widgetState === WidgetState.Ended && <div className="text-white scale-125 animate-fade-in"><svg className="h-20 w-20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M18.36 6.64a9 9 0 11-12.73 0M12 2v10" strokeLinecap="round" strokeLinejoin="round" /></svg></div>}
                                        {isOnline && isNetworkSlow && (widgetState === WidgetState.Speaking || widgetState === WidgetState.Listening) && <NetworkWarning />}
                                    </div>
                                </div>
                            </div>
                            <p className={`text-xl font-black px-6 text-center uppercase tracking-tight ${widgetState === WidgetState.Error ? 'text-red-600' : 'text-gray-900 dark:text-white'}`}>
                                {widgetState === WidgetState.Connecting ? "Connecting..." : widgetState === WidgetState.Listening ? "Listening..." : widgetState === WidgetState.Speaking ? "Speaking..." : widgetState === WidgetState.Error ? (errorMessage || "ERROR") : "Session Ended"}
                            </p>
                            <div className="h-24 mt-8 flex items-center justify-center">
                                {(widgetState === WidgetState.Connecting || widgetState === WidgetState.Listening || widgetState === WidgetState.Speaking) ? (
                                    <button onClick={endVoiceSession} className="w-16 h-16 rounded-full bg-red-500 text-white flex items-center justify-center shadow-2xl hover:scale-110 active:scale-90"><svg className="h-8 w-8 rotate-135" viewBox="0 0 20 20" fill="currentColor"><path d="M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.773a11.037 11.037 0 006.105 6.105l.774-1.548a1 1 0 011.059-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z" /></svg></button>
                                ) : (
                                    !(widgetState === WidgetState.Ended) && <button onClick={startVoiceSession} disabled={!isOnline} className={`w-16 h-16 rounded-full bg-accent-${accentColorClass} text-white flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 disabled:opacity-50`}><svg className="h-8 w-8" viewBox="0 0 20 20" fill="currentColor"><path d="M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.773a11.037 11.037 0 006.105 6.105l.774-1.548a1 1 0 011.059-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z" /></svg></button>
                                )}
                            </div>
                        </div>
                    </div>
