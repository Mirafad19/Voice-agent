...
export const AgentWidget: React.FC<AgentWidgetProps> = ({ agentProfile, apiKey, isWidgetMode, onSessionEnd }) => {
  ...
  const endChatSession = useCallback(() => {
    ...
    if (isWidgetMode) {
       // Optional: Add transcript to dashboard recordings if needed, but primarily for email reporting
       if (onSessionEnd) {
           onSessionEnd({
               ...newRecording,
               id: `chat-${now.getTime()}`,
  ...
  const renderHomeView = () => (
      <div className="flex flex-col h-full bg-white dark:bg-gray-900">
          {/* Hero Section */}
          <div className={`relative min-h-[150px] bg-gradient-to-br from-accent-${accentColorClass}-500 to-accent-${accentColorClass}-700 p-6 flex flex-col justify-end text-white overflow-hidden`}>
               <div className="absolute top-0 right-0 p-4 opacity-20">
                   <svg className="w-24 h-24" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 2a8 8 0 100 16 8 8 0 000-16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-1.667a1 1 0 000-1.664l-3-1.667z" clipRule="evenodd" /></svg>
               </div>
               <h3 className="relative z-10 text-sm font-bold uppercase tracking-wider opacity-80 mb-1">{agentProfile.name}</h3>
               <div className="relative z-10">
                   <h1 className="text-3xl font-bold mb-1">Hi <span className="animate-wave inline-block">👋</span></h1>
                   <p className="text-white/90 font-medium">How can we help you today?</p>
               </div>
          </div>

          {/* Action Area */}
          <div className="flex-1 p-4 -mt-4 relative z-20">
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl p-2 border border-gray-100 dark:border-gray-700 h-full flex flex-col gap-4">
                  
                  {/* Chat Input Trigger */}
                  <form onSubmit={handleHomeInputSubmit} className="relative mt-2">
                      <input 
                        type="text" 
                        placeholder="Ask a question..." 
                        value={chatInput}
                        onChange={(e) => setChatInput(e.target.value)}
                        className="w-full pl-4 pr-10 py-4 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-accent-${accentColorClass} focus:outline-none shadow-sm transition-all"
                      />
                      <button type="submit" className="absolute right-3 top-1/2 transform -translate-y-1/2 p-2 bg-white dark:bg-gray-600 rounded-lg text-gray-400 hover:text-accent-${accentColorClass} shadow-sm">
                          <SendIcon />
                      </button>
                  </form>
                  ...
                  <button 
                    ...
                    className={`w-full text-left p-4 rounded-xl bg-gradient-to-r from-accent-${accentColorClass}-500 to-accent-${accentColorClass}-600 text-white shadow-lg hover:shadow-xl transform hover:-translate-y-1 transition-all flex items-center gap-4 group`}
                  >
                      <div className="h-12 w-12 rounded-full bg-white/20 flex items-center justify-center backdrop-blur-sm group-hover:scale-110 transition-transform">
  ...
  const renderChatView = () => (
      <div className="flex flex-col h-full bg-gray-50 dark:bg-gray-900">
          {/* Header */}
          <div className={`bg-white dark:bg-gray-800 p-4 shadow-sm border-b border-gray-200 dark:border-gray-700 flex items-center justify-between`}>
              <div className="flex items-center gap-3">
                  <button onClick={handleBack} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition-colors">
              ...
              <button onClick={endChatSession} className="text-xs bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 px-3 py-1.5 rounded-full font-medium transition-colors">
                  End Chat
              </button>
          </div>
  ...
  const renderVoiceView = () => (
      <div className="flex flex-col h-full bg-white dark:bg-gray-900 relative">
          <button onClick={handleBack} className="absolute top-4 left-4 z-50 p-2 bg-white/10 backdrop-blur-md rounded-full text-gray-500 dark:text-white hover:bg-white/20 transition-colors">
                ...
                <div className="relative w-48 h-48 flex items-center justify-center mb-8">
                    ...
                    {(widgetState === WidgetState.Listening || widgetState === WidgetState.Speaking) && (
                        <div className="relative w-36 h-36 flex items-center justify-center">
                            ...
                            <div className={`relative w-24 h-24 rounded-full flex items-center justify-center transition-all duration-300 shadow-2xl bg-gradient-to-br from-accent-${accentColorClass}-400 to-accent-${accentColorClass}-600 scale-110`}>
  ...
  if (!isOpen) {
    const fabContent = (
      <div className={`${themeClass} relative`}>
        ...
        <button onClick={toggleWidget} className={`w-14 h-14 md:w-16 md:h-16 rounded-full bg-gradient-to-br from-accent-${accentColorClass}-400 to-accent-${accentColorClass}-600 shadow-lg shadow-accent-${accentColorClass}/30 flex items-center justify-center text-white transform hover:scale-105 transition-all duration-300`}>
