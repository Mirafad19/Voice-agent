...
export const AgentWidget: React.FC<AgentWidgetProps> = ({ agentProfile, apiKey, isWidgetMode, onSessionEnd }) => {
  ...
  const renderVoiceView = () => (
      <div className="flex flex-col h-full bg-white dark:bg-gray-900 relative w-full">
          {/* Permission Overlay */}
          {permissionRequested && (
              <div className="absolute inset-0 z-[60] bg-black/80 flex flex-col items-center pt-20 text-white text-center animate-fade-in backdrop-blur-sm">
                  <div className="animate-bounce mb-6">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 10l7-7m0 0l7 7m-7-7v18" />
                      </svg>
                  </div>
                  <h3 className="text-2xl font-bold px-8 mb-2">Microphone Access</h3>
                  <p className="text-lg opacity-90 px-8">Please tap <span className="font-bold text-accent-${accentColorClass}">Allow</span> to start talking.</p>
              </div>
          )}

          {/* Header */}
          <div className="absolute top-0 left-0 w-full z-50 p-4 flex items-center justify-between">
              <button onClick={handleBack} className="p-2 bg-gray-100/50 dark:bg-gray-800/50 backdrop-blur-md rounded-full text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">
                   <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
              </button>
              <span className="font-bold text-lg text-gray-800 dark:text-white truncate max-w-[200px]">{agentProfile.name}</span>
                ...
                <div className="relative w-48 h-48 flex items-center justify-center mb-8">
                    ...
                    {(widgetState === WidgetState.Listening || widgetState === WidgetState.Speaking) && (
                        <div className="relative w-36 h-36 flex items-center justify-center">
                            {/* Sonar waves only when Speaking */}
                            ...
                            <div className={`relative w-24 h-24 rounded-full flex items-center justify-center transition-colors duration-300 shadow-2xl bg-gradient-to-br from-accent-${accentColorClass} to-gray-800 scale-110`}>
